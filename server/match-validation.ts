...
export function validateMatchOnServer(input: MatchValidationInput) {
  ...
  const firstInnings = buildTeamInnings({
    ...
    battingPlayerIds:
      effectiveBattingFirstTeamId === "teamA" ? input.teamAPlayerIds : input.teamBPlayerIds,
    performances: input.performances,
    bowlingOvers:
      effectiveBattingFirstTeamId === "teamA" ? input.bowlingOvers.teamB : input.bowlingOvers.teamA,
